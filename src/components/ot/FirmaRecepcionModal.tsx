...
interface FirmaRecepcionModalProps {
  isOpen: boolean;
  ordenNumero: number;
  onCancel: () => void;
  onConfirm: (data: { firma: string }) => Promise<void> | void;
}

const MAX_FIRMA_BYTES = 500 * 1024; // 500 KB

export function FirmaRecepcionModal({
  isOpen,
  ordenNumero,
  onCancel,
  onConfirm,
}: FirmaRecepcionModalProps) {
  ...
  const puedeConfirmar = firmaTouched && !submitting;
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden animate-in fade-in zoom-in-95">
        {/* Header */}
        <div className="bg-gradient-to-r from-red-600 to-red-500 px-8 py-6 text-white relative">
          <button
            onClick={() => !submitting && onCancel()}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/80">
                Constancia de Recepción · OT #{ordenNumero}
              </p>
              <h2 className="text-xl font-bold tracking-tight">Firma del Cliente</h2>
            </div>
          </div>
        </div>
        ...
        <div className="px-8 py-6 space-y-5">
          <p className="text-sm text-gray-500 leading-relaxed font-bold">
            Por favor, solicite al cliente que firme en el recuadro inferior para dar conformidad de la recepción del equipo.
          </p>

          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1.5">
                <PenLine size={12} /> Firma del receptor (Cliente / Autorizado)
              </label>
              <button
                type="button"
                onClick={clearCanvas}
            ...
            <div className={`relative rounded-2xl border-2 border-dashed ${firmaTouched ? "border-red-400" : "border-gray-200"} bg-white overflow-hidden transition-all`}>
              <canvas
                ref={canvasRef}
        ...
        <div className="px-8 py-5 bg-gray-50 border-t border-gray-100 flex items-center justify-between gap-4">
          ...
          <button
            ...
            className="px-6 py-3 text-[10px] font-bold text-white rounded-xl uppercase tracking-widest transition-all shadow-md disabled:opacity-40 disabled:cursor-not-allowed bg-red-600 hover:bg-red-700"
          >
